...
class App extends Component {
  ...
  verifyNFT = () => {
    ...
    this.state.web3.eth.personal
      .sign(this.state.web3.utils.toHex(this.message), this.state.account)
      .then(signature => {
        // send signature and address to api
        axios
          .post("http://localhost:3004/api/verify", {
            signature: signature,
  ...
  fetchData = () => {
    ...
    // fetch data
    axios
      .get("http://localhost:3004/api/data", {
  ...
  render() {
    return (
      <div className="d-flex flex-column" style={fullscreen}>
        <Router>
          <Header web3={this.state.web3} connectFn={this.connectWallet} />
          <NFTCollection web3={this.state.web3}></NFTCollection>
          {/* <Route
            exact
            path="/"
            render={() => (
